...
def main():
    ...
    test_file = test_cfg.get("test_file")
    if not test_file or not os.path.exists(test_file):
        raise FileNotFoundError(f"test_file not found in config: {test_file}")

    cfg["test_image_dir"] = cfg.get("test_image_dir", cfg.get("image_dir"))
    if not cfg["test_image_dir"] or not os.path.isdir(cfg["test_image_dir"]):
        raise FileNotFoundError(f"test_image_dir not found: {cfg['test_image_dir']}")
    ...
    # 直接评测整份测试集
    acc = trainer.evaluate(tag="full_test")
    if trainer.accelerator.is_main_process:
        trainer.logger.INFO(f"[Done] Test accuracy: {acc:.4f}")
